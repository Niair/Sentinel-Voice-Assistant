...
async def test_1_connection():
    ...
    try:
        ...
        if client.is_available():
            ...
            print("[FAIL] Qdrant client not available")
            return False
...
async def test_2_collection_operations():
    ...
    try:
        client = get_qdrant_client()
        test_collection = "test_sentinel_collection_" + str(
        ...
        if not exists:
            print("[FAIL] Collection doesn't exist after creation")
            return False
        print("[PASS] Collection verified to exist")

        # Delete collection (cleanup)
        print(f"Cleaning up test collection...")
        await client.delete_collection(test_collection)
        print("[PASS] Collection deleted")

        return True

    except Exception as e:
        print(f"[FAIL] Collection operations test failed: {e}")
        import traceback
...
async def test_3_document_indexing():
    ...
    try:
        client = get_qdrant_client()
        test_collection = "test_rag_" + str(asyncio.get_event_loop().time()).replace(
            ".", "_"
        )
        ...
        if not result.get("success"):
            print(f"[FAIL] Indexing failed: {result.get('error')}")
            return False

        print(f"[PASS] Indexed {result['chunks_indexed']} documents")
        ...
        for query, expected_term in test_queries:
            ...
            if results:
                ...
                found_term = any(
                    expected_term.lower() in doc.page_content.lower() for doc in results
                )
                if found_term:
                    print(f"   [PASS] Found expected term '{expected_term}' in results")
                else:
                    print(
                        f"   [WARN] Expected term '{expected_term}' not found in top results"
                    )
                    all_passed = False
            else:
        ...
        print(f"[FAIL] Document indexing test failed: {e}")
        import traceback
...
async def test_4_retrieval_quality():
    ...
    try:
        client = get_qdrant_client()
        test_collection = "test_quality_" + str(
            asyncio.get_event_loop().time()
        ).replace(".", "_")
        ...
        print("\n[SEARCH] CRITICAL TEST: Finding author names")
        ...
        if not results:
            print("   [FAIL] FAILED: No results found for author query")
            await client.delete_collection(test_collection)
            return False
        ...
        for i, doc in enumerate(results, 1):
            ...
            if "Vaswani" in doc.page_content or "Shazeer" in doc.page_content:
                found_authors = True
                print("   [PASS] Contains author names!")

            if "@google.com" in doc.page_content or "Emails:" in doc.page_content:
                found_emails = True
                print("   [PASS] Contains email information!")
        ...
        if found_authors and found_emails:
            print(
                "\n[PASS] CRITICAL TEST PASSED: Successfully found authors and emails"
            )
            return True
        elif found_authors:
            print("\n[WARN] Partial success: Found authors but not emails")
            return True
        else:
            print("\n[FAIL] CRITICAL TEST FAILED: Could not find author information")
            return False

    except Exception as e:
        print(f"[FAIL] Retrieval quality test failed: {e}")
        import traceback
...
async def main():
    ...
    if not results[0][1]:
        print("\n[FAIL] Connection failed - stopping tests")
