...
try:
    ...
    print(f"⚠️ langchain-google-genai import error: {e}")
...
class SafeQdrantClient:
    ...
    def __init__(self):
        ...
        self._embedding_dim = 768  # Google text-embedding-004
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            print(f"✅ Qdrant RAG system initialized (URL: {self.qdrant_url})")
    ...
    async def index_documents(self, collection_name: str, documents: List[Document], batch_size: int = 50) -> Dict[str, Any]:
        """Index documents using upsert"""
        if not self.is_available(): return {"success": False, "error": "Client not available"}
    ...
    async def search(self, collection_name: str, query: str, limit: int = 6, score_threshold: float = 0.3) -> List[Document]:
        """
