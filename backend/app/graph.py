...
_current_thread_id = contextvars.ContextVar(
    "current_thread_id", default="default_thread"
)
...
async def process_document(pdf_path: str, thread_id: str = "default_thread"):
    ...
    try:
        ...
        # Split into chunks - LARGER chunks to reduce count and stay under daily quota
        splitter = RecursiveCharacterTextSplitter(
            chunk_size=2000,  # Increased from 800 to reduce chunk count (stays under 1000/day limit)
            chunk_overlap=200,  # Increased from 150 to maintain context
            length_function=len,
            separators=["\n\n", "\n", " ", ""],
        )
...
async def agent(state: ChatState, config: RunnableConfig, store: BaseStore):
    ...
    if load_mcp:
        try:
            ...
            print(
                f"💰 Loading {len(mcp_tools)} MCP tools (finance query detected in: '{last_user_msg[:60]}...')"
            )
        except Exception as e:
            print(f"⚠️ MCP tool initialization failed: {e}")
    else:
        print(
            f"📝 Using only static tools (no finance keywords in: '{last_user_msg[:50]}...')"
        )
    ...
    available_tools_text = (
        ...
        else "   (No MCP tools loaded for this query)"
    )
    ...
    if len(validated_messages) > max_messages:
        print(
            f"⚠️ Truncating message history from {len(validated_messages)} to {max_messages}"
        )
    ...
    try:
        ...
        if (
            "tool call validation failed" in error_msg
            and "expected number, but got string" in error_msg
        ):
            ...
            try:
                ...
                if last_user_msg and any(
                    keyword in last_user_msg.lower()
                    for keyword in ["expense", "add", "cost"]
                ):
                    ...
                    if amount_match:
                        ...
                        description_parts = re.split(
                            r"\b\d+(?:\.\d+)?\b", last_user_msg
                        )
                        description = " ".join(description_parts).strip()
                        description = re.sub(
                            r"\b(add|expense|for|the|today|of)\b",
                            "",
                            description,
                            flags=re.IGNORECASE,
                        ).strip()
                        ...
                        print(
                            f"🔧 Created manual tool call: amount={amount}, description='{description}'"
                        )
                        return {"messages": [tool_call_msg]}
                    ...
                    if tool_name == "add_expense":
                        if "amount" in args and isinstance(args["amount"], str):
                            try:
                                ...
                                print(
                                    f"🔧 Fixed add_expense amount: '{args['amount']}' -> {args['amount']}"
                                )
                            except ValueError:
                                print(
                                    f"⚠️ Could not convert amount '{args['amount']}' to number"
                                )
                                ...
                                if date_str and not date_str.count("-") == 2:
                                    ...
                                    try:
                                        if date_str.lower() == "today":
                                            args[date_param] = datetime.now().strftime(
                                                "%Y-%m-%d"
                                            )
                                        elif "/" in date_str:
